...
```{r Deseq2countmatrix, message=FALSE, warning=FALSE, paged.print=TRUE}
...
countData <- read.csv("C:/Users/DELL/Downloads/count_matrix.txt", header=T, row.names=1, sep="\t") 
dim(countData)
head(countData) 
...
heatmap.2( log2(counts(dds, normalized=TRUE)[rownames(dds) %in% selected, ]),
           ...
           dendrogram="row",
           trace="none",
           margins=c(5, 5),  # Adjust margin sizes as needed
